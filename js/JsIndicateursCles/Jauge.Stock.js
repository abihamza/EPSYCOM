	...
	return {
		...
		if (document.documentElement.clientWidth <= 414){
			var config = {
				...
				max: undefined != scope.max ? scope.max : localStorage.getItem("ngStorage-maxStkMois"),
				minorTicks: 4
			};
			}
		if (document.documentElement.clientWidth <= 736 && document.documentElement.clientWidth > 414){
			...
			config.redZones = [ { from: localStorage.getItem("ngStorage-greenStkMoisColor"), to: config.max } ];
			config.yellowZones = [ { from: localStorage.getItem("ngStorage-orangeStkMoisColor"), to: localStorage.getItem("ngStorage-greenStkMoisColor")} ];
			config.greenZones = [ { from: config.min + range*0, to: localStorage.getItem("ngStorage-orangeStkMoisColor")} ];
	...
	return {
		...
		link: function (scope, element, attrs, ngModelCtrl) {
			...
			var config = {
				...
				label: attrs.label,
				min: undefined != scope.min ? scope.min : 0,
				max: undefined != scope.max ? scope.max : localStorage.getItem("ngStorage-maxStkAnnee"),
				minorTicks: 4
			};
			}
		if (document.documentElement.clientWidth <= 736 && document.documentElement.clientWidth > 414){
			...
			var range = config.max - config.min;
			config.redZones = [ { from: localStorage.getItem("ngStorage-greenStkAnneeColor"), to: config.max } ];
			config.yellowZones = [ { from: localStorage.getItem("ngStorage-orangeStkAnneeColor"), to: localStorage.getItem("ngStorage-greenStkAnneeColor")} ];
			config.greenZones = [ { from: config.min + range*0, to: localStorage.getItem("ngStorage-orangeStkAnneeColor")} ];
